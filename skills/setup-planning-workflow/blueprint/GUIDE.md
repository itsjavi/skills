...
- `.specs/MANUAL_QA.md`: live manual QA coverage map: user journeys, roles, setup data, devices/browsers, acceptance
  signals, and stale or missing coverage. Update it when user-facing behavior changes.
...
flows and update them when user-facing behavior, roles, permissions, configuration, edge cases, or acceptance criteria
change.
...
validation plan linked to [CHECKS.md](CHECKS.md), manual QA impact linked to [MANUAL_QA.md](MANUAL_QA.md),
decision-record check, circuit breakers, risk notes, and a final **Final review pass** phase.
...
3. Check whether this phase made or changed a major durable architecture, product, security, operational, API strategy,
   ...
   decision records.
4. Update [MANUAL_QA.md](MANUAL_QA.md) when user-facing behavior, roles, permissions, edge cases, or acceptance criteria
   changed.
...
- If a change affects canonical automated verification, update [CHECKS.md](CHECKS.md).
- If a change affects user-facing workflows, roles, permissions, configuration, edge cases, or acceptance criteria,
  update [MANUAL_QA.md](MANUAL_QA.md).
...
- checks run
- notes or surprises
- implications for product specs, bug-fix records, decisions, automated checks, manual QA, or future plans
- follow-ups
